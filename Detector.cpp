...
Detector::Detector(InstrumentCalDataSet^ Calset)							
{
...
//		if (startI < 25)
		...
		float deltaFreq = (float)Cal->GetFreqFromPhaseCalGrid(qPeriod);
#endif
		diffQDelayNs = 0; //1e+9 * deltaPhase/(deltaFreq * 360.0f);  // differential time delay Q-phase compared to I-phase

		// Compute phase detector leading / laging asymmetry for I and Q
		Iasym = 180; //(firstImax-firstImin)*360/(secondImin-firstImin);
...
//		if (stopI > 1024)				// don't search past end of array
		...
		for (int i= startI; i<stopI; i++)
		{
			...
			phase += CompensateQDelay(phase, Cal->GetFreqFromPhaseCalGrid(i));
